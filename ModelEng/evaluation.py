...
def show_history(hist, save_path = None):
    """
    show the most important plots to a history and saves  them if desired

    :param hist: the history that should be investigated
    :param save_path: where to store the files+name
    """
    val_loss_sp = None
    acc_sp = None
    if save_path:
...
def compare_histories(hist_one, hist_two, hist_one_name, hist_two_name, compare_accuracy=True, compare_loss=True, save_path=None, step_size =1):
    """
    compares two histories

    :param hist_one:
    :param hist_two:
    :param hist_one_name:
    :param hist_two_name:
    :param compare_accuracy:
    :param compare_loss:
    :param save_path:
    :param step_size: step size of histories (make sure it is the same)
    """
    if compare_accuracy:
...
def get_all_model_iterations(model_name='pytk_rock_paper_scissors', dropouts=True, batch_norm=False,
    ...
    """
    if you trained a model and saved it after a certain amount of trainin steps, this function allows for a loading of all model iterations

    is connected to the naming scheme of the model training as implemented in the code only file

    :param model_name: the name of your model
    :param dropouts: dropouts used during training?
    :param batch_norm:  batch_norm was used ?
    :param start_iterations:
    :param stop_iterations:
    :param step: after which amount of training steps the model should be repeatedly saved
    :return: list of loaded pytorch models, ordered by the number of training steps (ascending)
    """
    model_paths = []
...
def test_accuracy_for_model_iterations(model_iterations, dataloader):
    """
    test the accuracy of a set of model iterations as generated by get_all_model_iterations on same data

    :param model_iterations: the list of pytorch models
    :param dataloader: a pytorch dataloader with the data you want to test on
    :return: list of tupels('model_name':..,'accuracy measures':...) -- accuracy measures for total, rock paper and scissors
    """
    hist = []
...
def load_data_from_pattern(file_pattern, batch_size=32, target_size=(64, 64), show_images=False):
    """
    creates a pytorch dataloader object based on a specific file pattern

    :param file_pattern: file pattern of the data
    :param batch_size: the batch size of the dataloader
    :param target_size: the target size of the data for model input
    :param show_images: if set, a plot is shown with a batch of data images
    :return: teh pytorch dataloader
    """
...
def max_acc_from_history(hist):
    """
    finds the maximum total accuracy in a history

    :param hist: a training history as generated by our model training
    :return: tuple: (index, maximum_value)
    """
    max = None
    index = None
...
def experiment_DataEng_background_removal(output_dir='./experiment_DataEng/', generate_data=False,
    ...
    """
    code for the experimient of the DataEng part

    :param output_dir: where output should be stored
    :param generate_data: has to be set true only the first time it is run, and creates all the necessary data for experiment
    :param show_data_loader_images: if set to True, used images are shown
    """
